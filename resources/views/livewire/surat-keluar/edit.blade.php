<div>
    <flux:button icon="edit" variant="subtle" data-modal-target="edit-{{ $suratKeluarId }}" data-modal-toggle="edit-{{ $suratKeluarId }}"></flux:button>
    <!-- Main modal -->
    <div wire:ignore.self id="edit-{{ $suratKeluarId }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-500 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <div class="flex items-center justify-between p-4 md:p-5">
                    ...
                    <flux:button wire:click="form.resetForm" icon="X" variant="subtle" data-modal-toggle="edit-{{ $suratKeluarId }}">
                    </flux:button>
                </div>
                <!-- Modal body -->
                <div class="p-4 md:p-5">
                    <form wire:submit.prevent="update" class="space-y-4" method="POST">
                        ...
                        <!-- Modal Confirmation -->
                        <div class="flex">
                            <flux:modal.trigger name="persetujuan_edit-{{ $suratKeluarId }}">
                                <flux:spacer />
                            ...
                            <flux:modal name="persetujuan_edit-{{ $suratKeluarId }}" class="md:w-96">
                                <div class="space-y-6">
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        <flux:modal.close>
                                            <flux:button variant="primary" color="green" type="submit" class="w-full" data-modal-toggle="edit-{{ $suratKeluarId }}">Simpan</flux:button>
                                        </flux:modal.close>    
                                        <flux:modal.close>                                         
                                            <flux:button variant="danger" wire:close="persetujuan_edit-{{ $suratKeluarId }}" class="w-full">Batal</flux:button>
                                        </flux:modal.close>
                                    </div>
                                </div>
                            </flux:modal>
